...
def load_wardrobe_csv(csv_path):
    wardrobe_df = pd.read_csv(csv_path, on_bad_lines="skip")
    required_columns = {"image_path", "category", "style"}

    if required_columns.issubset(wardrobe_df.columns):
        return wardrobe_df

    styles_columns = {"id", "articleType", "usage"}
    if styles_columns.issubset(wardrobe_df.columns):
        wardrobe_df = wardrobe_df.copy()
        wardrobe_df["image_path"] = wardrobe_df["id"].apply(
            lambda image_id: str(PROJECT_ROOT / "data" / "images" / f"{image_id}.jpg")
        )
        wardrobe_df["category"] = wardrobe_df["articleType"]
        wardrobe_df["style"] = wardrobe_df["usage"]
        return wardrobe_df.dropna(subset=["category", "style"])

    missing_columns = required_columns - set(wardrobe_df.columns)
    if missing_columns:
        missing = ", ".join(sorted(missing_columns))
        raise ValueError(
            "Wardrobe CSV is missing required column(s): "
            f"{missing}. Use image_path,category,style or the original styles.csv format."
        )
