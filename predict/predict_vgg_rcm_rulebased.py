...
def parse_args():
    ...
    parser.add_argument("image_path", nargs="?", default=str(ROOT / "test_images" / "redshirts.jpg"))
...
def show_outfit(outfit):
    ...
    for ax, (slot, item) in zip(axes, items):
        ...
        # Cắt ngắn tên nếu dài quá để đỡ bị tràn khung ảnh
        short_name = (name[:20] + '..') if len(name) > 20 else name
        ax.set_title(f"{slot}\n{short_name}", fontsize=10, fontweight='bold', color='navy')
        ax.axis("off")
    ...
    # LƯU ẢNH RA FILE ĐỂ XEM TRÊN COLAB
    output_filename = "outfit_result.png"
    plt.savefig(output_filename, dpi=150, bbox_inches='tight')
    print(f"\n[+] ĐÃ LƯU ẢNH OUTFIT: Nhìn sang cột Files bên trái, nháy đúp vào file '{output_filename}' để xem nhé!")
